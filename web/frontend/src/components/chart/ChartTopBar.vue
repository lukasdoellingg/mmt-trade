...
import { FUTURES_METRIC_LABELS } from '../../features/futures/futuresConstants';
import type { FuturesMetricKind } from '../../workspace/types';

const props = withDefaults(defineProps<{ mode?: 'heatmap' | 'futures' }>(), { mode: 'heatmap' });
...
function addWidgetByType(type: string) {
  ...
  else if (type === 'coin-scanner') {
    const v = viewportCells();
    const slot = findFreeSlot(28, 60, v.w, v.h);
    addWidget('coin-scanner', { x: slot.x, y: slot.y, w: 28, h: 60 });
  } else if (type === 'futures-metric-pane') {
    addMetricPane('funding');
...
function addMetricPane(metric: FuturesMetricKind) {
  const v = viewportCells();
  const slot = findFreeSlot(34, 28, v.w, v.h);
  addWidget('futures-metric-pane', { x: slot.x, y: slot.y, w: 34, h: 28 }, { metric });
}

function onAddMetricClick(e: Event) {
  e.stopPropagation();
  closeAllMenus();
  addMetricOpen.value = true;
}

function onLayoutSlot(slot: 1 | 2 | 3 | 4) {
...
function onIndicatorsClick(e: Event) {
  e.stopPropagation();
  closeAllMenus();
  indicatorsOpen.value = true;
}
function onOrderflowClick(e: Event) {
  e.stopPropagation();
  closeAllMenus();
  orderflowOpen.value = true;
}
function onAddWidgetClick(e: Event) {
  e.stopPropagation();
  closeAllMenus();
  addWidgetOpen.value = true;
}
</script>
...
<template>
  <div class="topbar">
    ...
    <span v-if="isFutures" class="tb-divider"></span>

    <div v-if="isFutures" class="layout-slots">
      <button
        v-for="slot in [1, 2, 3, 4]"
        :key="slot"
        class="slot-btn"
        :class="{ active: activeLayoutSlot === slot }"
        :title="`Layout ${slot}`"
        @click="onLayoutSlot(slot as 1 | 2 | 3 | 4)"
      >
        {{ slot }}
      </button>
    </div>
    ...
    <button
      ...
      class="ic-btn"
      :class="{ on: pane.obHeatmap }"
      title="Order Book heatmap"
      @click="pane.obHeatmap = !pane.obHeatmap"
    >
      <svg width="14" height="14" viewBox="0 0 14 14">
        <path fill="currentColor" d="M2 2h3v10H2zM5.5 5h3v7h-3zM9 8h3v4H9z" />
      </svg>
    </button>
    <button
      v-show="hasChart && !isFutures"
      class="ic-btn"
      :class="{ on: pane.footprint }"
      title="Footprint"
      @click="pane.footprint = !pane.footprint"
    >
      <svg width="14" height="14" viewBox="0 0 14 14">
        <path fill="currentColor" d="M2 3h4v2H2zm6 0h4v2H8zM2 6h4v2H2zm6 0h4v2H8zM2 9h4v2H2zm6 0h4v2H8z" />
      </svg>
    </button>
    <button
      v-show="hasChart && !isFutures"
      ...
      :class="{ on: pane.vpvr }"
      title="VPVR"
      @click="pane.vpvr = !pane.vpvr"
    >
      <svg width="14" height="14" viewBox="0 0 14 14">
        <path fill="currentColor" d="M11 2h2v10h-2zM8 5h2v7H8zM5 7h2v5H5zM2 9h2v3H2z" />
      </svg>
    </button>
    <button
      v-show="hasChart && !isFutures"
      class="ic-btn"
      :class="{ on: pane.liquidations }"
      title="Liquidations"
      @click="pane.liquidations = !pane.liquidations"
    >
      <svg width="14" height="14" viewBox="0 0 14 14">
        <circle cx="3.5" cy="6" r="2" fill="currentColor" />
        <circle cx="10" cy="9" r="2.5" fill="currentColor" opacity=".8" />
      </svg>
    </button>
    ...
    <div v-show="hasChart" class="dd-wrap">
      <button ref="indicatorsBtn" class="dd-btn" @click="onIndicatorsClick">
        Indicators <span class="dd-caret">&#9662;</span>
      </button>
    </div>

    <button
      class="ic-btn"
      :class="{ on: shell.tool === 'pencil' }"
      title="Drawing tool"
      @click="togglePencil"
    >
      <svg width="14" height="14" viewBox="0 0 14 14">
        <path fill="currentColor" d="M11.5 1.5l1 1L4 11l-1.5.5L3 10z" />
      </svg>
    </button>

    <button
      class="ic-btn"
      :class="{ on: shell.quoteUsd }"
      title="Toggle quote currency"
      @click="toggleQuoteUsd"
    >
      {{ shell.quoteUsd ? '$ USD' : 'BASE' }}
    </button>
    <button class="ic-btn" title="Fullscreen" @click="toggleFullscreen">
      <svg width="14" height="14" viewBox="0 0 14 14">
        <path
          fill="currentColor"
          d="M2 2h4v1.5H3.5V6H2zm10 0v4h-1.5V3.5H8V2zM2 12V8h1.5v2.5H6V12zm10 0H8v-1.5h2.5V8H12z"
        />
      </svg>
    </button>
    ...
    <div v-if="isFutures" class="dd-wrap">
      <button ref="addMetricBtn" class="dd-btn" @click="onAddMetricClick">
        + Metric <span class="dd-caret">&#9662;</span>
      </button>
    </div>

    <div v-show="!isFutures" class="dd-wrap">
      <button ref="orderflowBtn" class="dd-btn" @click="onOrderflowClick">
        Order Flow <span class="dd-caret">&#9662;</span>
      </button>
    </div>
    ...
    <button class="ic-btn" title="Screenshot" @click="takeScreenshot">
      <svg width="14" height="14" viewBox="0 0 14 14">
        <path fill="currentColor" d="M3 4h2l1-1.5h2L9 4h2v7H3z" />
        <circle cx="7" cy="7.5" r="2" fill="#06060b" />
      </svg>
    </button>
    <div class="dd-wrap">
      <button
    ...
    <button class="ic-btn" title="Settings" @click="openSettings">
      <svg width="14" height="14" viewBox="0 0 14 14">
        <path
          fill="currentColor"
          d="M7 4.5A2.5 2.5 0 1 0 9.5 7 2.5 2.5 0 0 0 7 4.5zM12 7l-1-.5.3-1.3-1.1-1.1L9 4.4 8.5 3.4l-1.5 0L6.5 4.4 5 4.6 3.8 5.7l.3 1.3L3 7l1 .5-.3 1.3 1.1 1.1L6 9.6l.5 1H7.5l.5-1L9.5 9 10.7 7.9 10.4 6.7z"
        />
      </svg>
    </button>
  </div>
  ...
  <Teleport to="body">
    <div v-if="indicatorsOpen" class="dd-float dd-menu" :style="indicatorsPos" @click.stop>
      <label class="dd-row"><input v-model="pane.vwapDaily" type="checkbox" /> VWAP Daily</label>
      <label class="dd-row"><input v-model="pane.vwapWeekly" type="checkbox" /> VWAP Weekly</label>
      <label class="dd-row"><input v-model="pane.vwapMonthly" type="checkbox" /> VWAP Monthly</label>
      <label class="dd-row"><input v-model="pane.vwapBands" type="checkbox" /> VWAP &#963; bands</label>
      <label class="dd-row"><input v-model="pane.ema" type="checkbox" /> EMA 9/21</label>
      <label class="dd-row"><input v-model="pane.liquidations" type="checkbox" /> Liquidations</label>
      <label class="dd-row"><input v-model="pane.obHeatmap" type="checkbox" /> OB Heatmap</label>
      <label class="dd-row"><input v-model="pane.footprint" type="checkbox" /> Footprint</label>
      <label class="dd-row"><input v-model="pane.vpvr" type="checkbox" /> VPVR</label>
      <span class="dd-hint">Server scripts (/ws/session)</span>
      <label class="dd-row"
        ><input v-model="pane.scriptKeyLevels" type="checkbox" :disabled="!USE_SESSION_MUX" /> Key
        Levels</label
      >
      <label class="dd-row"
        ><input v-model="pane.scriptNetPositioning" type="checkbox" :disabled="!USE_SESSION_MUX" /> Net
        Positioning</label
      >
      <label class="dd-row"
        ><input v-model="pane.scriptObImbalance" type="checkbox" :disabled="!USE_SESSION_MUX" /> OB
        Imbalance</label
      >
    </div>
    ...
    <div v-if="addMetricOpen" class="dd-float dd-menu" :style="addMetricPos" @click.stop>
      <button
        v-for="m in FUTURES_METRICS"
        :key="m"
        class="dd-row"
        @click="
          addMetricPane(m);
