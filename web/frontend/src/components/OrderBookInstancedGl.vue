...
watch([bidRows, askRows, () => props.step, () => props.mode], () => {
  syncOdinWidths();
  nextTick(scrollPinned);
}, { flush: 'post' });

watch(() => props.loading, (ld) => {
  if (!ld) nextTick(() => { syncOdinWidths(); scrollPinned(); });
});

watch(() => props.bids, () => { syncOdinWidths(); nextTick(scrollBidsTop); }, { flush: 'post' });
watch(() => props.asks, () => { syncOdinWidths(); nextTick(scrollAsksDown); }, { flush: 'post' });
...
<style scoped>
.ob{background:#0f0f14;border:1px solid #1e2a1e;border-radius:4px;overflow:hidden;flex:1 1 0;min-height:180px;display:flex;flex-direction:column}
...
.ob-err{padding:10px 12px;background:#2a1018;border-bottom:1px solid #5a2020;color:#f0a0a8;font-size:.72rem;flex-shrink:0}
.ob-err-title{font-weight:600;color:#ffb0b8;margin-bottom:4px}
.ob-err-body{margin:0;white-space:pre-wrap;word-break:break-word;font-family:Consolas,"Courier New",monospace;font-size:.68rem;color:#e8c0c4}
.ob-main{position:relative;flex:1;display:flex;flex-direction:column;min-height:0;overflow:hidden}
.ob-loading{position:absolute;inset:0;z-index:3;display:flex;align-items:center;justify-content:center;background:rgba(15,15,20,0.82);color:#5a7a5a;font-size:1rem;pointer-events:none}
.ob-body-dim{opacity:0.35;pointer-events:none}
...
.asks,.bids{flex:1;overflow-y:auto;overflow-x:hidden;min-height:80px;position:relative;background:#0f0f14}
...
.ob-row{position:relative;display:grid;grid-template-columns:1fr auto;align-items:center;column-gap:8px;padding:0 10px;font:0.75rem Consolas,"Courier New",monospace;content-visibility:auto;contain-intrinsic-size:17px 100%}
.ob-barWrap{position:absolute;inset:0;pointer-events:none;display:flex}
.ob-barWrap.ask{justify-content:flex-end;padding-right:10px}
.ob-barWrap.bid{justify-content:flex-start;padding-left:10px}
.ob-bar{height:100%;max-height:15px;align-self:center;border-radius:1px;opacity:0.24}
.ob-bar.ask{background:#ef4f60}
.ob-bar.bid{background:#3dc985}
.ob-p{z-index:1;font-weight:500}
.ob-p.ask{color:#ef4f60;text-align:left}
.ob-p.bid{color:#3dc985;text-align:left}
.ob-q{z-index:1;color:#8ab88a;text-align:right}
