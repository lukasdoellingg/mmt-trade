...
function go() {
  if (symbol.value)
    emit('select', { exchange: exchange.value, symbol: symbol.value, timeframe: timeframe.value });
}
</script>
    ...
    <div v-if="!started" class="landing">
      <div class="logo">
        <svg viewBox="0 0 120 120" class="logo-svg">
          ...
          <polygon
            points="60,16 97,38 97,82 60,104 23,82 23,38"
            fill="none"
            stroke="#1e3a1e"
            stroke-width="1.5"
          />
          ...
          <defs>
            <radialGradient id="glow">
              <stop offset="0%" stop-color="#3dc985" stop-opacity=".12" />
...
.logo-svg {
  width: 100%;
  height: 100%;
  filter: drop-shadow(0 0 24px rgba(61, 201, 133, 0.15));
}
.brand {
  margin: 0;
  font-size: 1.6rem;
...
.start-btn:hover {
  background: #1a2e1a;
  border-color: #3dc985;
  box-shadow: 0 0 20px rgba(61, 201, 133, 0.15);
}
