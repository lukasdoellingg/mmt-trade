    ...
    <div v-else class="body">
      ...
      <div ref="asksEl" class="asks" @scroll="onAsksScroll">
        <div
          ...
          :key="a[0]"
          class="row ask"
          :style="{ '--pct': Math.min(100, (a[1] / stats.mxA) * 100) }"
        >
          <span class="p">{{ fmtP(a[0]) }}</span
          ><span class="s">{{ fmtS(a[1], a[0]) }}{{ sfx }}</span>
        </div>
      </div>
      ...
      <div ref="bidsEl" class="bids" @scroll="onBidsScroll">
        <div
          ...
          :key="b[0]"
          class="row bid"
          :style="{ '--pct': Math.min(100, (b[1] / stats.mxB) * 100) }"
        >
          <span class="p">{{ fmtP(b[0]) }}</span
          ><span class="s">{{ fmtS(b[1], b[0]) }}{{ sfx }}</span>
        </div>
      </div>
    </div>
  </div>
...
.ask::before {
  background: rgba(239, 79, 96, 0.2);
}
.bid::before {
  background: rgba(61, 201, 133, 0.2);
}
.p,
.s {
  position: relative;
