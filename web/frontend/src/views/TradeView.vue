...
function startStreams() {
  ...
  obCleanup = createAllOrderbooksWs(props.symbol, pushOb, {
    onLoaded: (exId) => {
      obLoading.value = { ...obLoading.value, [exId]: false };
    },
  });
}
...
watch(
  () => props.symbol,
  () => {
    if (active) startStreams();
  },
);
watch(
  () => [props.symbol, props.timeframe, props.exchange],
  () => {
    if (active) loadVwap();
  },
);
  ...
  <div class="trade">
    ...
    <aside class="sidebar">
      <div class="vwap">
        ...
        <div class="vwap-row">
          <span class="lbl">D</span><span class="val">{{ fmtV(vwap.d) }}</span>
        </div>
        <div class="vwap-row">
          <span class="lbl">W</span><span class="val">{{ fmtV(vwap.w) }}</span>
        </div>
        <div class="vwap-row">
          <span class="lbl">M</span><span class="val">{{ fmtV(vwap.m) }}</span>
        </div>
      </div>
      <div class="ob-cfg">
        <select v-model.number="step" class="sel">
        ...
        <select v-model="obMode" class="sel">
...
.ob-list::-webkit-scrollbar-track {
  background: #0f0f14;
}
