...
/**
 * mmt.gg-style workspace shell.
 *
 *   ┌────────────────────────── topbar ──────────────────────────┐
 *   │ rail │            WorkspaceGrid (chart + ladders)          │
 *   │      │                                                     │
 *   └─────┴──────────────────────────────────────────────────────┘
 */
...
function bootDefaults(): void {
  ...
  // First-time defaults expressed in cells of CELL_PX. Proportions:
  // chart = 78% width, ladders share remaining 22% × full height (50/50 stack).
  ...
  ensureDefaults([
    ...
    { type: 'orderflow-ladder', rect: { x: chartW, y: 0, w: ladW, h: halfH },
      props: { aggregate: 'binance,bybit,okx,coinbase,deribit,kraken,bitfinex' } },
    { type: 'orderflow-ladder', rect: { x: chartW, y: halfH, w: ladW, h: hCells - halfH },
      props: { aggregate: 'binancef,bybitf,okxf,deribitf,bitmexf,hyperliquid' } },
  ]);
}
